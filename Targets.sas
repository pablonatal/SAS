...
%macro getPopulation(
	...
	*********************** *********************** *********************** */
	...
	proc sql;
		create table work.pop_&ano_char4.&mes_char4. as
			select 	CUST_ID,
					ARPU as	ARPU_A1
	...
	/* Mes de análisis */
	proc sql;
		create table work.pop_&ano_char3.&mes_char3. as
			select 	CUST_ID,
					ARPU,
					MIN_TRAF_IN_TOTAL,
					MIN_TRAF_TOTAL,
	...
	******************************** *
	...
	proc sql;
	 create table work.medias_p as
		select CUST_ID, sum(ARPU_M1+ARPU_M2)/2 as ARPU_avg_pop
		from work.medias_p
		group by CUST_ID
	quit;
	...
	******* */
	...
	proc sort data=	work.pop_&ano_char3.&mes_char3.; by CUST_ID; run;
	proc sort data=	work.pop_&ano_char4.&mes_char4.; by CUST_ID; run;


	data work.targets_&ano_char3.&mes_char3.;
	 merge 	work.pop_&ano_char3.&mes_char3.(in=a)
			work.pop_&ano_char1.&mes_char1.(in=b)  /* Mes M+1 */
			work.pop_&ano_char2.&mes_char2.(in=c)  /* Mes M+2 */
			work.pop_&ano_char4.&mes_char4. (in=d) /* MEs M-1 */
	
		;
		by CUST_ID;
		if a /* Poblacion de analisis */ then do;
			...
			/*Downsell*/
			if (a and b and c) and ( (ARPU_M1-ARPU) < (-0.20*ARPU)) then TARGET_DOWN_20PCT_M1 = 1; else TARGET_DOWN_20PCT_M1 = 0;
			if (a and b and c) and ( (ARPU_M1-ARPU) < 0 and (ARPU_M2-ARPU) < (-0.20*ARPU)) then TARGET_DOWN_20PCT_M2 = 1; else TARGET_DOWN_20PCT_M2 = 0;

			if (a and b and c) and ( (ARPU_M1-ARPU) < (-0.50*ARPU)) then TARGET_DOWN_50PCT_M1 = 1; else TARGET_DOWN_50PCT_M1 = 0;
			if (a and b and c) and ( (ARPU_M1-ARPU) < 0 and (ARPU_M2-ARPU) < (-0.50*ARPU)) then TARGET_DOWN_50PCT_M2 = 1; else TARGET_DOWN_50PCT_M2 = 0;

			if (a and b and c and d) and ( sum(ARPU_A1+ARPU)/2 > 1.2*(sum(ARPU_M1+ARPU_M2)/2)) and (ARPU_M1<ARPU_A1) and (ARPU_M1<ARPU) and (ARPU_M2<ARPU_A1) and (ARPU_M2<ARPU) then TARGET_DOWN_20PCT_AVG = 1; else TARGET_DOWN_20PCT_AVG = 0;
			if (a and b and c and d) and ( sum(ARPU_A1+ARPU)/2 > 1.5*(sum(ARPU_M1+ARPU_M2)/2)) and (ARPU_M1<ARPU_A1) and (ARPU_M1<ARPU) and (ARPU_M2<ARPU_A1) and (ARPU_M2<ARPU) then TARGET_DOWN_50PCT_AVG = 1; else TARGET_DOWN_50PCT_AVG = 0;


			/* Upsell */
			if (a and b and c) and ( (ARPU-ARPU_M1) < (-0.20*ARPU_M1)) then TARGET_UP_20PCT_M1 = 1; else TARGET_UP_20PCT_M1 = 0;
			if (a and b and c) and ( (ARPU < ARPU_M1) and (ARPU_M2-ARPU) > (0.20*ARPU)) then TARGET_UP_20PCT_M2 = 1; else TARGET_UP_20PCT_M2 = 0;

			if (a and b and c) and ( (ARPU-ARPU_M1) < (-0.50*ARPU_M1)) then TARGET_UP_50PCT_M1 = 1; else TARGET_UP_50PCT_M1 = 0;
			if (a and b and c) and ( (ARPU < ARPU_M1) and (ARPU_M2-ARPU) > (0.50*ARPU)) then TARGET_UP_50PCT_M2 = 1; else TARGET_UP_50PCT_M2 = 0;

			if (a and b and c and d) and ( (sum(ARPU_A1+ARPU)/2)*1.2 < sum(ARPU_M1+ARPU_M2)/2) and (ARPU_M1>ARPU_A1) and (ARPU_M1>ARPU) and (ARPU_M2>ARPU_A1) and (ARPU_M2>ARPU) then TARGET_UP_20PCT_AVG = 1; else TARGET_UP_20PCT_AVG = 0;
			if (a and b and c and d) and ( (sum(ARPU_A1+ARPU)/2)*1.5 < sum(ARPU_M1+ARPU_M2)/2) and (ARPU_M1>ARPU_A1) and (ARPU_M1>ARPU) and (ARPU_M2>ARPU_A1) and (ARPU_M2>ARPU) then TARGET_UP_50PCT_AVG = 1; else TARGET_UP_50PCT_AVG = 0;

			/* Portado */
			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 ne 'NATIVO' and PORTADO_M2 ne 'NATIVO' then TARGET_PORTADO_M2 = 1; else  TARGET_PORTADO_M2 = 0;
			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 eq 'IN' and PORTADO_M2 eq 'IN' then TARGET_PORTADO_IN_M2 = 1; else  TARGET_PORTADO_IN_M2 = 0;
			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 eq 'OUT' and PORTADO_M2 eq 'OUT' then TARGET_PORTADO_OUT_M2 = 1; else  TARGET_PORTADO_OUT_M2 = 0;

			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 ne 'NATIVO' then TARGET_PORTADO_M1 = 1; else  TARGET_PORTADO_M1 = 0;
			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 eq 'IN'  then TARGET_PORTADO_IN_M1 = 1; else  TARGET_PORTADO_IN_M1 = 0;
			if (a and b and c) and PORTADO = 'NATIVO' and PORTADO_M1 eq 'OUT' then TARGET_PORTADO_OUT_M1 = 1; else  TARGET_PORTADO_OUT_M1 = 0;
	...
	***********	*/
	...
	create table work.summary as
	 select &ano_char3.&mes_char3. as yearmonth,
			...
			sum(TARGET_DOWN_20PCT_M1)  as TARGET_DOWN_20PCT_M1,
			sum(TARGET_DOWN_20PCT_M2)  as TARGET_DOWN_20PCT_M2,
			sum(TARGET_DOWN_20PCT_AVG) as TARGET_DOWN_20PCT_AVG,
			sum(TARGET_DOWN_50PCT_M1)  as TARGET_DOWN_50PCT_M1,
			sum(TARGET_DOWN_50PCT_M2)  as TARGET_DOWN_50PCT_M2,
			sum(TARGET_DOWN_50PCT_AVG) as TARGET_DOWN_50PCT_AVG,

			sum(TARGET_UP_20PCT_M1)  as TARGET_UP_20PCT_M1,
			sum(TARGET_UP_20PCT_M2)  as TARGET_UP_20PCT_M2,
			sum(TARGET_UP_20PCT_AVG) as TARGET_UP_20PCT_AVG,
			sum(TARGET_UP_50PCT_M1)  as TARGET_UP_50PCT_M1,
			sum(TARGET_UP_50PCT_M2)  as TARGET_UP_50PCT_M2,
			sum(TARGET_UP_50PCT_AVG) as TARGET_UP_50PCT_AVG,
			
			sum(TARGET_PORTADO_M1) as TARGET_PORTADO_M1,
			sum(TARGET_PORTADO_IN_M1) as TARGET_PORTADO_IN_M1,
			sum(TARGET_PORTADO_OUT_M1) as TARGET_PORTADO_OUT_M1,
			sum(TARGET_PORTADO_M2) as TARGET_PORTADO_M2,
			sum(TARGET_PORTADO_IN_M2) as TARGET_PORTADO_IN_M2,
			sum(TARGET_PORTADO_OUT_M2) as TARGET_PORTADO_OUT_M2

		from work.targets_&ano_char3.&mes_char3.
	;
	quit;
